/*
Решить задачи из практикума №15 II, используя методы расширения
...
class Program
{
    static void Main()
    {
        StreamReader fileIn = new StreamReader("d:/MetodRasshireniya2/input.txt");
        ...
        fileIn.Close();
        var result = students.Where(x => x.exam1 == 2 || x.exam2 == 2 || x.exam3 == 2)
            .GroupBy(x => x.faculty)
            .Select(stud => new { Faculty = stud.Key, Students = stud });
        StreamWriter fileOut = new StreamWriter("d:/MetodRasshireniya2/output.txt");
